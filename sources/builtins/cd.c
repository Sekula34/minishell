...
//return -1 if fails 
//return 0 if ok
int	value_setter(char *key, char *new_value, t_vars **ex_h, t_vars **env_head)
{
	t_vars	*old_element_ex;
	t_vars	*old_element_env;

	old_element_ex = get_element(key, *ex_h);
	...
	if (create_add_new_elems(key, new_value, ex_h, env_head) == 0)
		return (0);
	return (-1);
}
